...
export function LogsPage({ initialKind = 'core' }: { initialKind?: LogKind }) {
  ...
  const previousStreamURL = useRef(streamURL)
  ...
  useEffect(() => {
    if (previousStreamURL.current !== streamURL) {
      setEntries([])
    ...
    source.addEventListener('log', (event) => {
      try {
        ...
        // Reconnects replay the bounded server snapshot. Keep paused history
        // visible and merge that replay instead of clearing or duplicating it.
        setEntries((current) => mergeLogEntry(current, entry))
      } catch {
  ...
  const filtered = entries.filter((entry) =>
    (level === 'all' || (level === 'warn' ? ['warn', 'warning'].includes(entry.level.toLocaleLowerCase()) : entry.level.toLocaleLowerCase() === level)) &&
  ...
  return <>
    <PageHeader title={t('logs')} actions={<>
      <button className="du-btn du-btn-ghost du-btn-sm" onClick={clearView}>{t('clearView')}</button>
    ...
    <div className="log-stream-controls"><div className="stream-state" role="status"><span className={streamState === 'open' ? 'status-dot good' : streamState === 'stopped' ? 'status-dot' : 'status-dot warning'} />{streamState === 'stopped' ? t('stopped') : streamState === 'reconnecting' ? t('reconnecting') : t('live')}</div>
    ...
    <div className="panel log-filters">
      ...
      <label>{t('logLevel')}<select className="du-select du-select-sm" value={level} onChange={(event) => setLevel(event.currentTarget.value)}><option value="all">{t('all')}</option><option value="debug">debug</option><option value="info">info</option><option value="warn">warn</option><option value="error">error</option></select></label>
    ...
    {!streamReady && entries.length === 0 && <Loading />}
    {streamReady && filtered.length === 0 && <Empty>{entries.length > 0 ? t('noSearchResults') : t('noData')}</Empty>}
...
export function mergeLogEntry(entries: LogEntry[], entry: LogEntry): LogEntry[] {
  const duplicate = entries.some((current) => current.time === entry.time
    && current.level === entry.level
    && current.component === entry.component
    && current.message === entry.message
    && JSON.stringify(current.fields) === JSON.stringify(entry.fields))
  return duplicate ? entries : [...entries.slice(-299), entry]
}
